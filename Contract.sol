...
contract JokInTheBoxStaking is Ownable {
    ...
    address public stakingSigner = 0x80E4929c869102140E69550BBECC20bEd61B080c;
     ...
     */
    constructor() {
        jokToken = IERC20(0x7a33B6C083778E9e8019919561e0da0cF9E46B75);
    ...
    */
    function withdraw(uint256 earnings, uint256 affiliateEarnings, bool inETH, uint8 _v, bytes32 _r, bytes32 _s) external {
        // Validate the ECDSA signature for secure withdrawal
        uint256 totalEarnings = earnings + affiliateEarnings;
        require(isValidSignature(msg.sender, totalEarnings, inETH, _v, _r, _s), "Invalid signature!");
    ...
    */
     ...
     */
    function isValidSignature(address beneficiary, uint256 amount, bool inETH, uint8 _v, bytes32 _r, bytes32 _s) internal view returns (bool) {
        // Compute the hash of the transaction data
        bytes memory hash = abi.encodePacked(address(this), beneficiary, amount, inETH, nonce[beneficiary]);
        // Add the Ethereum signed message prefix to the hash
        ...
        // Validate that the recovered signer matches the staking signer
